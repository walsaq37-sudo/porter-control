(function(){
  ...
  async function renderSupervisorDashboard(){
    ...
    const summaries=porters.map(n=>{const r=rounds.filter(x=>x.member_name===n&&x.inspection_type==='Round completed').length;const insp=rounds.filter(x=>x.member_name===n&&x.inspection_type!=='Round completed').length;const c=checks.filter(x=>x.member_name===n).length;return `<div class="review"><strong>👷 ${n}</strong><div class="muted">Rounds: ${r} · Inspections: ${insp} · Containers: ${c}</div></div>`}).join('');
