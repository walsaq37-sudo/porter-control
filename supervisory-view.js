(function(){
  ...
  async function renderSupervisorDashboard(){
    ...
    const summaries=porters.map(n=>{const mine=rounds.filter(x=>x.member_name===n);const r=mine.filter(isRound).length;const insp=mine.filter(isInspection).length;const fallbackContainers=mine.filter(isContainer).length;const directContainers=checks.filter(x=>x.member_name===n).length;const c=directContainers+fallbackContainers;return `<div class="review"><strong>👷 ${n}</strong><div class="muted">Rounds: ${r} · Inspections: ${insp} · Containers: ${c}</div></div>`}).join('');
