const CACHE='porter-control-v28';
...
self.addEventListener('fetch',e=>{
  if(e.request.mode==='navigate'){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{
      ...
      if(!html.includes('shared-containers.js'))html=html.replace('</body>','<script src="shared-containers.js?v=3"></script></body>');
      if(!html.includes('team-shifts.js'))html=html.replace('</body>','<script src="team-shifts.js?v=2"></script></body>');
      if(!html.includes('supervisory-view.js'))html=html.replace('</body>','<script src="supervisory-view.js?v=4"></script></body>');
