const CACHE='porter-control-v35';
...
self.addEventListener('fetch',e=>{
  if(e.request.mode==='navigate'){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{
      ...
      if(!html.includes('team-shifts.js'))html=html.replace('</body>','<script src="team-shifts.js?v=3"></script></body>');
      if(!html.includes('supervisory-view.js'))html=html.replace('</body>','<script src="supervisory-view.js?v=6"></script></body>');
